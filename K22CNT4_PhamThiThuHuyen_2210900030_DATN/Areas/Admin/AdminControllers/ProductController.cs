    ...
    public class ProductController : BaseAdminController
    {
        ...
        )
        {
            var products = _context.Products
                ...
                .Include(p => p.ProductVariants).ThenInclude(v => v.Material)
        ...
        public IActionResult Create(Product model, List<IFormFile> images, int? defaultImage)
        {
            ...
            model.Gender = (model.Gender == 2 || model.Gender == 3) ? model.Gender : 1;
            model.CreatedDate = DateTime.Now;
            model.Isdelete = 0;

            if (string.IsNullOrEmpty(model.Slug))
            {
                model.Slug = model.Name?.ToLower().Trim().Replace(" ", "-");
            }
        ...
        public IActionResult Edit(long id)
        {
            ...
            ViewBag.Sizes = _context.Sizes.Where(s => s.Isactive == 1).ToList();
